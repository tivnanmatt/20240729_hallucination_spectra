...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.bn1 = nn.BatchNorm2d(32)  # Batch Norm after the first conv layer
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm2d(64)  # Batch Norm after the second conv layer
...
def test_and_plot_conf_matrix(model, test_loader):
    ...
    y_true = []
    y_pred = []
    image_sets_a_d = torch.load("samples_200_clip/image_sets_a_d.pt")
    labels = torch.load("samples_200_clip/ground_truth_label.pt")
    true_images, measurements, reconstructions = image_sets_a_d
    recons = reconstructions[:, 0, 0, :, :, :]
    with torch.no_grad():
        # for n in range(16):
        #     label = labels[n]
        #     recon = recons[n]
    ...
    plt.title('Confusion Matrix for CNN MNIST Classifier with Batch Norm')
    plt.savefig('cnn_mnist_confusion_matrix_bn_recon.png')
    plt.show()
