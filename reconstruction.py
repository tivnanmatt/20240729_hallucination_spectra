...
class ImageReconstructionTask(nn.Module):
    ...
    def train_reconstructor(self, *args, repeat, num_epochs=100, num_iterations=100, optimizer=None, verbose=True, **kwargs):
...
class DiffusionBridgeModel(ImageReconstructionTask):
    ...
    def train_diffusion_backbone(self, *args, repeat, num_epochs=100, num_iterations=100, optimizer=None, verbose=True, time_sampler=None,**kwargs):

        # plot loss vs. total iterations
        def plot_loss(loss_arr):
            ...
            plt.show()
            plt.savefig(f"/home/local/PARTNERS/yk792/laboratory/python/projects/20240702_hallucination/loss_plot_0730/loss_plot.png")
        ...
        for epoch in range(num_epochs):
            ...
            avg_train_loss /= num_iterations
            loss_arr[epoch] = avg_train_loss
            if verbose:
                print(f"Epoch {epoch}: Loss: {loss.item()}")
